...
<div class="space-y-1.5">
  ...
  <div class="quill-wrapper">
    <QuillEditor
      v-if="cargadoInicial"
      :key="editorKey"
      v-model:content="form.aprendizajes_esperados"
      contentType="html"
      :options="editorOptions"
      theme="snow"
    />
...
<div class="space-y-1.5">
  ...
  <div class="quill-wrapper">
    <QuillEditor
      v-if="cargadoInicial"
      :key="editorKey"
      v-model:content="form.criterios"
      contentType="html"
      :options="editorOptions"
      theme="snow"
    />
  ...
  <div class="space-y-1.5">
    ...
    <div class="quill-wrapper">
      <QuillEditor
        v-if="cargadoInicial"
        :key="editorKey"
        v-model:content="form.diagnostico"
        contentType="html"
        :options="editorOptions"
        theme="snow"
      />
  ...
  <div class="space-y-1.5">
    ...
    <div class="quill-wrapper">
      <QuillEditor
        v-if="cargadoInicial"
        :key="editorKey"
        v-model:content="form.bibliografia"
        contentType="html"
        :options="editorOptions"
        theme="snow"
      />
...
watch(() => props.initialData, (newData) => {
    ...
    if (props.isEdit && newData) {
      ...
      form.value.areas_id = source.areas_id || contenido.areas_id || ''
      form.value.persona_cargo_cursado_id = source.persona_cargo_cursado_id || contenido.persona_cargo_cursado_id || ''
      form.value.tipo_planificacion = source.tipo_planificacion || contenido.tipo_planificacion || 'Anual'
      ...
      // Asignación segura leyendo de la raíz o de .contenido indistintamente
      form.value.saberes = limpiarTexto(source.saberes ?? contenido.saberes)
      form.value.aprendizajes_esperados = limpiarTexto(source.aprendizajes_esperados ?? contenido.aprendizajes_esperados)
      form.value.criterios = limpiarTexto(source.criterios ?? contenido.criterios)
      form.value.diagnostico = limpiarTexto(source.diagnostico ?? contenido.diagnostico)
      form.value.bibliografia = limpiarTexto(source.bibliografia ?? contenido.bibliografia)
...
const editorOptions = {
  ...
  placeholder: 'Redacte el desarrollo didáctico aquí...'
}
